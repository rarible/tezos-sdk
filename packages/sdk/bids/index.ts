...
export async function accept_bid(provider: Provider, bid_data: AcceptBid) : Promise<OperationResult> {
  const owner = await get_address(provider)
  const arg = accept_bid_arg(provider, bid_data)
...
function accept_bid_arg(
  ...
  return { destination: provider.config.bid, entrypoint: "accept_bid", parameter };
}